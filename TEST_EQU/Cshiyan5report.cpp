...
BEGIN_MESSAGE_MAP(Cshiyan5report, CDialogEx)
	ON_BN_CLICKED(IDC_BUTTON1, &Cshiyan5report::OnBnClickedButton1)
	ON_BN_CLICKED(IDC_BUTTON2, &Cshiyan5report::OnBnClickedButton2)
	ON_BN_CLICKED(IDC_BUTTON3, &Cshiyan5report::OnBnClickedButton3)
END_MESSAGE_MAP()


// Cshiyan5report 消息处理程序
CString Cshiyan5report::VariantToCString(_variant_t var)
{
...
BOOL Cshiyan5report::OnInitDialog()
{
...
void Cshiyan5report::OnBnClickedButton2()
{
	...
	UpdateData(TRUE);                       //这个函数的使用请参考数据交换UpdateData，那篇文章  
	int index = m_comboxs5.GetCurSel();       //这个函数用于得到用户选择的是下拉列表中的第几行，第一行的话，返回0，依次类推  
	CString strI;
	strI.Format(_T("%d"), index);
	CString strC;
	m_comboxs5.GetLBText(index, strC);        //根据行号，这个可以得到那行的字符串  
	CString sqlorder;
	sqlorder = strC;
	CString strSQL; //存放sql语句
