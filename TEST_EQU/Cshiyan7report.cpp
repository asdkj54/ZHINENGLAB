...
BEGIN_MESSAGE_MAP(Cshiyan7report, CDialogEx)
	ON_BN_CLICKED(IDC_BUTTON1, &Cshiyan7report::OnBnClickedButton1)
	ON_BN_CLICKED(IDC_BUTTON2, &Cshiyan7report::OnBnClickedButton2)
	ON_BN_CLICKED(IDC_BUTTON3, &Cshiyan7report::OnBnClickedButton3)
END_MESSAGE_MAP()


// Cshiyan7report 消息处理程序
CString Cshiyan7report::VariantToCString(_variant_t var)
{
...
BOOL Cshiyan7report::OnInitDialog()
{
...
void Cshiyan7report::OnBnClickedButton1()
{
	// TODO: 在此添加控件通知处理程序代码
...
void Cshiyan7report::OnBnClickedButton2()
{
	// TODO: 在此添加控件通知处理程序代码
	if (!m_DataBase.Open("Provider=SQLOLEDB;Server=172.20.42.6,1433;Database=text;uid=30039;pwd=30039621;"))
		return;
	...
	UpdateData(TRUE);                       //这个函数的使用请参考数据交换UpdateData，那篇文章  
	int index = m_comboxs7.GetCurSel();       //这个函数用于得到用户选择的是下拉列表中的第几行，第一行的话，返回0，依次类推  
	CString strI;
	strI.Format(_T("%d"), index);
	CString strC;
	m_comboxs7.GetLBText(index, strC);        //根据行号，这个可以得到那行的字符串  
	CString sqlorder;
	sqlorder = strC;
	CString strSQL; //存放sql语句
...
void Cshiyan7report::OnBnClickedButton3()
{
	// TODO: 在此添加控件通知处理程序代码
	if (!m_DataBase.Open("Provider=SQLOLEDB;Server=172.20.42.6,1433;Database=text;uid=30039;pwd=30039621;"))
		return;
