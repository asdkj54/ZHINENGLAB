...
IMPLEMENT_DYNAMIC(Ctext6, CDialogEx)

Ctext6::Ctext6(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_text6, pParent)
{
...
void Ctext6::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(Ctext6, CDialogEx)
	ON_BN_CLICKED(IDC_BUTTON1, &Ctext6::OnBnClickedButton1)
	ON_BN_CLICKED(IDC_BUTTON2, &Ctext6::OnBnClickedButton2)
	ON_BN_CLICKED(IDC_BUTTON4, &Ctext6::OnBnClickedButton4)
END_MESSAGE_MAP()


// Ctext6 消息处理程序
CString Ctext6::VariantToCString(_variant_t var)
{
...
void Ctext6::OnBnClickedButton2()
{
	...
	sqlorder = strC;
	CString strSQL; //存放sql语句
	strSQL.Format(_T("select * from test6 where 工作台 = '%s' order by 日期 desc"), sqlorder);
	vector<vector<_variant_t>> vResult(m_DataBase.Select(::SysAllocString(strSQL), vName));
	m_listshiyan6.DeleteAllItems();	//删除所有的项目
	//通过循环添加所有的内容
	for (int i = 0; i < vResult.size(); ++i)
