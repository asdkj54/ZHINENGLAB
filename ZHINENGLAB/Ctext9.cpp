...
IMPLEMENT_DYNAMIC(Ctext9, CDialogEx)

Ctext9::Ctext9(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_text9, pParent)
{
...
void Ctext9::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(Ctext9, CDialogEx)
	ON_BN_CLICKED(IDC_BUTTON1, &Ctext9::OnBnClickedButton1)
	ON_BN_CLICKED(IDC_BUTTON2, &Ctext9::OnBnClickedButton2)
	ON_BN_CLICKED(IDC_BUTTON4, &Ctext9::OnBnClickedButton4)
END_MESSAGE_MAP()


// Ctext9 消息处理程序
CString Ctext9::VariantToCString(_variant_t var)
{
...
void Ctext9::OnBnClickedButton1()
{
	// TODO: 在此添加控件通知处理程序代码
...
void Ctext9::OnBnClickedButton2()
{
	// TODO: 在此添加控件通知处理程序代码
	if (!m_DataBase.Open("Provider=SQLOLEDB;Server=192.168.240.165,1433;Database=text;uid=30039;pwd=30039621;"))
		return;
	...
	sqlorder = strC;
	CString strSQL; //存放sql语句
	strSQL.Format(_T("select * from test9 where 工作台 = '%s' order by 日期 desc"), sqlorder);
	vector<vector<_variant_t>> vResult(m_DataBase.Select(::SysAllocString(strSQL), vName));
	m_listshiyan9.DeleteAllItems();	//删除所有的项目
	//通过循环添加所有的内容
	for (int i = 0; i < vResult.size(); ++i)
...
void Ctext9::OnBnClickedButton4()
{
	// TODO: 在此添加控件通知处理程序代码
	if (!m_DataBase.Open("Provider=SQLOLEDB;Server=192.168.240.165,1433;Database=text;uid=30039;pwd=30039621;"))
		return;
