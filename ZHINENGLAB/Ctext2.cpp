...
IMPLEMENT_DYNAMIC(Ctext2, CDialogEx)

Ctext2::Ctext2(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_text2, pParent)
{
...
BEGIN_MESSAGE_MAP(Ctext2, CDialogEx)
	ON_BN_CLICKED(IDC_BUTTON1, &Ctext2::OnBnClickedButton1)
	ON_BN_CLICKED(IDC_BUTTON2, &Ctext2::OnBnClickedButton2)
	ON_BN_CLICKED(IDC_BUTTON4, &Ctext2::OnBnClickedButton4)
END_MESSAGE_MAP()


// Ctext2 消息处理程序

CString Ctext2::VariantToCString(_variant_t var)
{
...
void Ctext2::OnBnClickedButton1()
{
	// TODO: 在此添加控件通知处理程序代码
...
void Ctext2::OnBnClickedButton2()
{
	// TODO: 在此添加控件通知处理程序代码
	if (!m_DataBase.Open("Provider=SQLOLEDB;Server=172.20.42.6,1433;Database=text;uid=30039;pwd=30039621;"))
		return;
	...
	sqlorder = strC;
	CString strSQL; //存放sql语句
	strSQL.Format(_T("select * from test2 where 工作台 = '%s' order by 日期 desc"), sqlorder);
	vector<vector<_variant_t>> vResult(m_DataBase.Select(::SysAllocString(strSQL), vName));
	m_listshiyan2.DeleteAllItems();	//删除所有的项目
	//通过循环添加所有的内容
	for (int i = 0; i < vResult.size(); ++i)
...
void Ctext2::OnBnClickedButton4()
{
	// TODO: 在此添加控件通知处理程序代码
	if (!m_DataBase.Open("Provider=SQLOLEDB;Server=172.20.42.6,1433;Database=text;uid=30039;pwd=30039621;"))
		return;
