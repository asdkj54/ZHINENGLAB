...
IMPLEMENT_DYNAMIC(Ctext4, CDialogEx)

Ctext4::Ctext4(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_text4, pParent)
{
...
void Ctext4::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_COMBO3, m_comboxs2);
	DDX_Control(pDX, IDC_COMBO1, m_comboxs1);
	DDX_Control(pDX, IDC_LIST1, m_listshiyan4);
}


BEGIN_MESSAGE_MAP(Ctext4, CDialogEx)
	ON_BN_CLICKED(IDC_BUTTON1, &Ctext4::OnBnClickedButton1)
	ON_BN_CLICKED(IDC_BUTTON2, &Ctext4::OnBnClickedButton2)
	ON_BN_CLICKED(IDC_BUTTON4, &Ctext4::OnBnClickedButton4)
END_MESSAGE_MAP()


// Ctext4 消息处理程序

CString Ctext4::VariantToCString(_variant_t var)
{
...
void Ctext4::OnBnClickedButton2()
{
	...
	sqlorder = strC;
	CString strSQL; //存放sql语句
	strSQL.Format(_T("select * from test4 where 工作台 = '%s' order by 日期 desc"), sqlorder);
	vector<vector<_variant_t>> vResult(m_DataBase.Select(::SysAllocString(strSQL), vName));
	m_listshiyan4.DeleteAllItems();	//删除所有的项目
	//通过循环添加所有的内容
	for (int i = 0; i < vResult.size(); ++i)
